...
| WebSocket ACT/resume | Unit/fake socket: HELLO/AUTH/ACT/OBSERVE, persist resume_token, HELLO resume restores session, reconnect after drop. Isolated `--transport websocket` fail-closed (`WS_ISOLATED`). Live Perihelion WS ACT not re-run (HTTP proof already OBSERVED). |
